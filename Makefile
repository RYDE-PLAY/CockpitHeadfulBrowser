...
SPEC=$(RPM_NAME).spec
PREFIX ?= /usr/local
APPSTREAMFILE=org.cockpit_project.browser.metainfo.xml
...
deb: $(NODE_MODULES_TEST) $(COCKPIT_REPO_STAMP)
	NODE_ENV=production ./build.js
	./scripts/build-deb.sh

check-backend:
	python3 -m unittest discover -s test/backend -v

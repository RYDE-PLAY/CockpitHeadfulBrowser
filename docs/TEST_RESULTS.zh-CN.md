...
- 新增移动端 WebKit 回归：iPhone 13 模拟视口下，noVNC 内层屏幕和 canvas 均有非零尺寸，页面和控制台无错误。修复了 WebKit 将 noVNC `height: 100%` 解析为零导致移动端空白的问题；此项仍不替代真实 iOS 设备验收。
...
- 已验证中文剪贴板输入和 WebKit 移动视口渲染；原生中文输入法、真实 iOS 触摸/全屏、Firefox 客户端、AMD64 和其他发行版尚未验收。
